...
{
private:
    vector<DataPoint1> dadaSets;        //数据集合
    ...
    double GetDistance(DataPoint1& dp1, DataPoint1& dp2);                    //距离函数
    void SetArrivalPoints(DataPoint1& dp);                                //设置数据点的领域点列表
    void KeyPointCluster( unsigned long i, unsigned long clusterId );    //对数据点领域内的点执行聚类操作
public:

    ClusterAnalysis1(){}                    //默认构造函数
    bool Init(double radius, int minPTs);    //初始化操作
    int DoDBSCANRecursive();            //DBSCAN递归算法
    int WriteToFile();    //将聚类结果写入文件
...
double ClusterAnalysis1::GetDistance(DataPoint1& dp1, DataPoint1& dp2)
{
    ...
    for(int i=0; i < DIME_NUM1; i++)    //对数据每一维数据执行
    {
