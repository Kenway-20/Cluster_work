...
{
private:
    vector<DataPoint3> dadaSets;        //数据集合
    ...
    double GetDistance(DataPoint3& dp1, DataPoint3& dp2);                    //距离函数
    void SetArrivalPoints(DataPoint3& dp);                                //设置数据点的领域点列表
    void KeyPointCluster( unsigned long i, unsigned long clusterId );    //对数据点领域内的点执行聚类操作
public:

    ClusterAnalysis3(){}                    //默认构造函数
...
int ClusterAnalysis3::DoDBSCANRecursive()
{
    ...
    {
        DataPoint3& dp=dadaSets[i];                    //取到第i个数据点对象
...
{
    DataPoint3& srcDp = dadaSets[dpID];        //获取数据点对象
    ...
    {
        DataPoint3& desDp = dadaSets[arrvalPoints[i]];    //获取领域内点数据点
...
double ClusterAnalysis3::GetDistance(DataPoint3& dp1, DataPoint3& dp2)
{
    ...
    for(int i=0; i < DIME_NUM3; i++)    //对数据每一维数据执行
    {
