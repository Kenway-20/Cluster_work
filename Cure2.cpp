...
void  Cure2::BuildClustList(){
    ...
    for(i=0; i < NUMPATTERNS2; i++){
        ...
        for(j=0; j < SIZEVECTOR2; j++){
            p[i]->Pre[0][j]=Pattern[i][j]+ (SHRINK2) * (p[i]->Means[j] - Pattern[i][j]);//计算代表点
        }
    }
...
ClustNode2 * Cure2::createNode(ClustNode2 *u, ClustNode2 *v){
    int i,j;
    ClustNode2 *w=new ClustNode2;//创建新的簇
...
void Cure2::ShowClusters(){
    ...
    //cout<<"该数据集分为"<<NUMCLUSTERS2<<"类"<<endl;
    int right3 = 0;
    int right2 = 0;
    int right1 = 0;
